...
# =====================================================================================
# PHẦN 2: MÃ NGUỒN XÁC THỰC (TỪ kbum1.py)
# =====================================================================================

# CONFIGURATION
FREE_CACHE_FILE = 'free_key_cache.json'      # File cache cho key free
VIP_CACHE_FILE = 'vip_cache.json'            # File cache cho key VIP
HANOI_TZ = pytz.timezone('Asia/Ho_Chi_Minh') # Múi giờ Hà Nội
VIP_KEY_URL = "https://raw.githubusercontent.com/DUONGKP2401/KEY-VIP.txt/main/KEY-VIP.txt" # URL chứa danh sách key VIP
...
══════════════════════════
Admin: DUONG phung
Tool BUMX FB-TDK- hỗ trợ proxy-VIP
══════════════════════════
"""
...
def main_authentication():
    ...
    # 1. Ưu tiên kiểm tra key VIP đã lưu
    ...
    # 2. Nếu không có key VIP, kiểm tra key free đã lưu trong ngày
    ...
    # 3. Nếu không có key nào được lưu, hiển thị menu lựa chọn
    while True:
...
# =====================================================================================
# PHẦN 3: MÃ NGUỒN TOOL CHÍNH (TỪ TDK.py)
# =====================================================================================
...
def check_proxy(proxy):
    """Kiểm tra proxy qua kiemtraip.vn"""
...
def rotate_proxy():
    """Xoay sang proxy tiếp theo và kiểm tra live"""
...
def facebook_info(cookie: str, proxy: str = None, timeout: int = 15):
    try:
        ...
        if proxy:
            ...
            session.proxies = None # Đảm bảo proxy được dọn sạch
        ...
        # SỬA LỖI: Dùng 1 User-Agent nhất quán
        ...
        headers = {
            ...
            "sec-ch-ua": '"Not_A Brand";v="8", "Chromium";v="120", "Google Chrome";v="120"', # Cập nhật cho Chrome 120
            "sec-ch-ua-mobile": "?0",
            ...
            "user-agent": user_agent, # SỬ DỤNG BIẾN Ở ĐÂY
            "viewport-width": "1366",
            "Cookie": cookie
        }
        ...
        response_obj = session.get(url, headers=headers, params={"locale": "vi_VN"}, timeout=timeout)

        data_split = response_obj.text.split('"CurrentUserInitialData",[],{')
        ...
        elif "828281030927956" in response_obj.text:
            print("[!] Account is under a 956 checkpoint.")
            return {'success': False}
        elif "1501092823525282" in response_obj.text:
            print("[!] Account is under a 282 checkpoint.")
            return {'success': False}
        elif "601051028565049" in response_obj.text:
        ...
        json_data = {
            ...
            'cookie': cookie,
            'headers': headers, # Giữ lại headers gốc nếu cần
            'user_agent': user_agent # SỬA LỖI: Lưu UA để dùng nhất quán
        }
        return json_data
...
def get_post_id(session, cookie, link, user_agent): # Thêm user_agent
    ...
    headers = {
        ...
        'priority': 'u=0, i',
        'sec-ch-prefers-color-scheme': 'light',
        'sec-ch-ua': '"Not_A Brand";v="8", "Chromium";v="120", "Google Chrome";v="120"', # Cập nhật
        'sec-ch-ua-mobile': '?0',
        'sec-ch-ua-platform': '"Windows"',
        'sec-fetch-dest': 'document',
        'sec-fetch-mode': 'navigate',
        ...
        'sec-fetch-user': '?1',
        'upgrade-insecure-requests': '1',
        'user-agent': user_agent, # SỬA LỖI: Dùng UA nhất quán
        'cookie': cookie,
    }
    try:
    ...
    # SỬA LỖI: Xử lý lỗi kết nối cụ thể
    except requests.exceptions.RequestException as e:
        print(Fore.RED+f'Lỗi kết nối khi lấy ID post: {e}')
        return {'success':False}
    except Exception as e:
        print(Fore.RED+f'Lỗi không xác định khi lấy ID post: {e}')
        return {'success':False}
...
def react_post_perm(data,object_id,type_react, proxy=None):
    ...
    headers = {
        ...
        'user-agent': data['user_agent'], # SỬA LỖI: Dùng UA nhất quán
    ...
    try:
        # SỬA LỖI: Xử lý logic tắt proxy
        if proxy:
            data['session'].proxies = to_requests_proxies(proxy)
        else:
            data['session'].proxies = None
            
        response = data['session'].post('https://www.facebook.com/api/graphql/', headers=headers, data=json_data, timeout=15)
        response.raise_for_status()
        
        # SỬA LỖI: Xử lý lỗi JSONDecodeError
        response_json = response.json() 
        
        if response_json.get('data', {}).get('feedback_react') is not None:
            return True
        else:
            prints(255, 0, 0, f"Lỗi khi thả cảm xúc (perm): Phản hồi không hợp lệ. {response.text[:100]}")
            return False
    except requests.exceptions.HTTPError as http_err:
        prints(255, 0, 0, f"Lỗi HTTP khi thả cảm xúc (perm): {http_err} - {response.text[:100]}")
        return False
    except json.JSONDecodeError:
        # Đây là lỗi trong ảnh: "Expecting value: line 1 column 1 (char 0)"
        prints(255, 0, 0, f"Lỗi giải mã JSON khi thả cảm xúc (perm). Có thể là checkpoint/block. Phản hồi: {response.text[:100]}")
        return False
    except Exception as e:
        prints(255, 0, 0, f"Lỗi không xác định khi thả cảm xúc (perm): {e}")
        return False
...
def react_post_defaul(data,object_id,type_react, proxy=None):
    ...
    try:
        ...
        if response_json.get('data', {}).get('feedback_react') is not None:
            ...
            prints(255, 0, 0, f"Lỗi khi thả cảm xúc (default): Phản hồi không hợp lệ. {response.text[:100]}")
            return False
    except requests.exceptions.HTTPError as http_err:
        prints(255, 0, 0, f"Lỗi HTTP khi thả cảm xúc (default): {http_err} - {response.text[:100]}")
        return False
    except json.JSONDecodeError:
        prints(255, 0, 0, f"Lỗi giải mã JSON khi thả cảm xúc (default). Có thể là checkpoint/block. Phản hồi: {response.text[:100]}")
        return False
    except Exception as e:
        prints(255, 0, 0, f"Lỗi không xác định khi thả cảm xúc (default): {e}")
        return False
...
def react_stories(data,object_id, proxy=None):
    ...
    try:
        ...
        if response_json.get('data', {}).get('story_send_reply') is not None:
            return True
        else:
            prints(255, 0, 0, f"Lỗi khi react story: Phản hồi không hợp lệ. {response.text[:100]}")
            return False
    except requests.exceptions.HTTPError as http_err:
        prints(255, 0, 0, f"Lỗi HTTP khi react story: {http_err} - {response.text[:100]}")
        return False
    except json.JSONDecodeError:
        prints(255, 0, 0, f"Lỗi giải mã JSON khi react story. Có thể là checkpoint/block. Phản hồi: {response.text[:100]}")
        return False
    except Exception as e:
        prints(255, 0, 0, f"Lỗi không xác định khi react story: {e}")
        return False

def react_post(data,link,type_react, proxy=None):
    # SỬA LỖI: Truyền user_agent
    res_object_id=get_post_id(data['session'],data['cookie'],link, data['user_agent'])
...
def comment_fb(data,object_id,msg, proxy=None):
    ...
    try:
        ...
        # Kiểm tra phản hồi chi tiết hơn
        if 'data' in response_json and 'comment_create' in response_json['data'] and 'feedback_comment_edge' in response_json['data']['comment_create']:
            comment_node = response_json['data']['comment_create']['feedback_comment_edge'].get('node', {})
            comment_text = comment_node.get('preferred_body', {}).get('text')
            
            if comment_text is not None and comment_text == msg:
                prints(5,255,0,f'Đã comment "{comment_text}"',end='\r')
                return True
        
        prints(255,0,0,f"Lỗi khi comment: Phản hồi không xác nhận được comment. {response.text[:100]}")
        return False
    except requests.exceptions.HTTPError as http_err:
        prints(255, 0, 0, f"Lỗi HTTP khi comment: {http_err} - {response.text[:100]}")
        return False
    except json.JSONDecodeError:
        prints(255, 0, 0, f"Lỗi giải mã JSON khi comment. Có thể là checkpoint/block. Phản hồi: {response.text[:100]}")
        return False
    except Exception as e:
        prints(255,0,0,f"Lỗi không xác định khi comment: {e}")
        return False
...
def dexuat_fb(data,object_id,msg, proxy=None):
    ...
    try:
        ...
        # Kiểm tra phản hồi chi tiết hơn
        if 'data' in response_json and 'story_create' in response_json['data'] and 'profile_review_edge' in response_json['data']['story_create']:
        ...
        prints(255,0,0,f"Lỗi khi đánh giá Fanpage: Phản hồi không hợp lệ. {response.text[:100]}")
        return False
    except requests.exceptions.HTTPError as http_err:
        prints(255, 0, 0, f"Lỗi HTTP khi đánh giá Fanpage: {http_err} - {response.text[:100]}")
        return False
    except json.JSONDecodeError:
        prints(255, 0, 0, f"Lỗi giải mã JSON khi đánh giá Fanpage. Có thể là checkpoint/block. Phản hồi: {response.text[:100]}")
        return False
    except Exception as e:
        prints(255,0,0,f'Lỗi không xác định khi đánh giá Fanpage: {e}')
        return False
...
def reload(session, authorization, type_job, retries=3):
    prints(255, 255, 0, f'Đang nhấn tải danh sách nhiệm vụ {type_job}...', end='\r')
    if retries == 0:
...
def lam_job(data,jobs,type_job_doing, current_proxy=None):
    ...
    if type_job_doing=='review_facebook':
        # SỬA LỖI: Truyền user_agent
        res_get_post_id=get_post_id(data['session'],data['cookie'],link, data['user_agent'])
...
def get_lin_share(data,link, proxy=None):
    ...
    try:
        ...
        response = requests.post('https://www.facebook.com/api/graphql/',  headers=headers, data=payload, proxies=proxies, timeout=15)
        response.raise_for_status()
        
        # SỬA LỖI: Xử lý lỗi JSONDecodeError
        response_json = response.json()
        
        return response_json['data']['xfb_create_share_url_wrapper']['share_url_wrapper']['wrapped_url']
    except requests.exceptions.HTTPError as http_err:
        prints(255, 0, 0, f"Lỗi HTTP khi lấy link share: {http_err} - {response.text[:100]}")
        return ''
    except json.JSONDecodeError:
        prints(255, 0, 0, f"Lỗi giải mã JSON khi lấy link share. Phản hồi: {response.text[:100]}")
        return ''
    except Exception as e:
        prints(255,0,0,f'Lỗi không xác định khi lấy link share của post: {e}')
        return ''
...
def main_bumx_free():
    ...
    # TẠO SESSION RIÊNG CHO BUMX (KHÔNG PROXY)
...
Nhập STT các loại NV cần làm (ví dụ: 12 để làm cảm xúc và comment): ''',end='')
    ...
    if current_proxy:
        ...
        prints(0,255,255,f'🔗 Đang sử dụng proxy: {current_proxy}')
        ...
        try:
            ...
            if current_proxy and not check_proxy_fast(current_proxy):
                ...
                if current_proxy:
                    ...
                    # SỬA LỖI: Cập nhật proxy trên session
                    data['session'].proxies = to_requests_proxies(current_proxy)
                else:
                    prints(255,0,0,'❌ Không còn proxy live, tiếp tục không proxy.')
                    current_proxy = None
                    # SỬA LỖI: Xóa proxy khỏi session
                    data['session'].proxies = None
            
            # Chuyển đổi cookie FB khi đạt ngưỡng hoặc khi có lỗi
            if (tasks_on_current_cookie >= switch_threshold or force_switch_cookie) and len(valid_cookies) > 1:
                ...
                # Vòng lặp tìm cookie live tiếp theo
                while not new_data:
                    ...
                    if not valid_cookies:
                        ...
                        sys.exit(1) # Thoát hẳn nếu không còn cookie
                    
                    # Đảm bảo index không vượt quá giới hạn sau khi xóa
                    current_cookie_index = current_cookie_index % len(valid_cookies)
                    new_data = switch_facebook_account(valid_cookies[current_cookie_index], authorization, bumx_session, current_proxy)
                
                # Khi đã tìm thấy cookie live
                data = new_data
                tasks_on_current_cookie = 0
                force_switch_cookie = False # Reset cờ

            if not list_type_job:
                prints(5,255,0,'Đã hết loại nhiệm vụ để làm.')
                break
            
            # Lấy nhiệm vụ theo từng loại một
            for type_job in list_type_job:
                ...
                # Chỉ lấy những job thuộc loại đang làm
                all_jobs = get_job(bumx_session, authorization)
                jobs_to_do = [job for job in all_jobs if job['type'] == type_job]

                if not jobs_to_do:
                    prints(255,255,0,f'Không tìm thấy nhiệm vụ cho loại {type_job}, chuyển loại tiếp theo...', end='\r')
                    time.sleep(2)
                    continue
                ...
                for job in jobs_to_do:
                    ...
                    try:
                        ...
                        if res_load and res_load.get('success'):
                            delay = random.randint(delay1, delay2)
                            
                            # Thực hiện hành động trên Facebook
                            status_job = lam_job(data, res_load, job['type'], current_proxy)
                            
                            if status_job:
                                # Chờ 15s để kiểm tra xem có bị gỡ/ẩn không
                                prints(0, 191, 255, "Thao tác Facebook thành công, chờ 15s để xác minh...")
                                countdown(15)

                                res_submit = submit(bumx_session, authorization, job, status_job, res_load)
                                if res_submit[0]:
                                    demht += 1
                                    ...
                                    # Submit thất bại cũng nên báo lỗi job
                                    raise Exception("Submit thất bại, có thể do thao tác bị Facebook gỡ")
                            else:
                                # Nếu lam_job thất bại, báo lỗi và chuyển cookie
                                raise Exception("Hành động (react/comment) trên Facebook thất bại")

                    except Exception as e:
                        prints(255, 165, 0, f"NV lỗi ({e}); báo lỗi và bỏ qua job này")
                        report(bumx_session, authorization, job)
                        demsk += 1
                        force_switch_cookie = True # Đặt cờ để chuyển cookie ở vòng lặp tiếp theo
                        time.sleep(4)
                        if len(valid_cookies) > 1: # Nếu còn cookie khác, thoát khỏi vòng lặp job hiện tại để đổi cookie
                           break 
            
            # Nếu không tìm thấy nhiệm vụ nào trong tất cả các loại, đợi một lúc
            if not get_job(bumx_session, authorization):
                 prints(255,255,0,'Không tìm thấy nhiệm vụ nào, chờ 10 giây...', end='\r')
                 time.sleep(10)
...
if __name__ == "__main__":
    try:
        # Bước 1: Chạy xác thực từ kbum1.py
        is_authenticated = main_authentication()

        # Bước 2: Nếu xác thực thành công, chạy tool chính từ TDK.py
        if is_authenticated:
            print(f"\n{luc}Xác thực thành công. Bắt đầu chạy tool chính...{trang}")
            time.sleep(2)
            main_bumx_free()
        else:
            # Các thông báo lỗi đã được in bên trong hàm main_authentication
            print(f"\n{do}Xác thực không thành công. Dừng chương trình.{trang}")
            sys.exit()

    except Exception as e:
        # Nếu có bất kỳ lỗi không mong muốn nào xảy ra ở bất kỳ đâu,
        # chương trình sẽ hiển thị thông báo chung và thoát.
        # Điều này ngăn chặn việc lộ traceback hoặc mã nguồn.
        print(f"\n{do}Tool đang bị lỗi, xin chờ...{trang}")
        # Ghi lại lỗi vào file để debug (tùy chọn, có thể xóa dòng này)
