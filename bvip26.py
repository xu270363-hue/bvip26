...
# DANH SÁCH TỪ KHÓA NHẠY CẢM ĐỂ KIỂM TRA BÌNH LUẬN
...
def _parse_graphql_response(response):
    """Phân tích phản hồi API GraphQL của Facebook để kiểm tra thành công hoặc các lỗi cụ thể."""
    try:
        ...
        if 'data' in response_json and response_json.get('data'):
            # Kiểm tra xem có trường nào null không, điều này đôi khi cho thấy một lỗi một phần
...
def comment_fb(data, object_id, msg, proxy=None):
    ...
    try:
        ...
        if parsed_result['status'] == 'success':
            try:
                # Kiểm tra phản hồi để đảm bảo bình luận được tạo
...
#    """Kiểm tra xem một bình luận có thực sự hiển thị trên bài viết không."""
#    ... (Hàm này đã bị xóa vì không đáng tin cậy)
# ======================
...
# ================== SỬA LỖI ==================
# Sửa hàm get_job để chấp nhận type_job, cho phép lọc job ở phía API
def get_job(session, authorization, type_job=None):
    ...
    # THÊM: Thêm type_job vào params nếu nó được cung cấp
    if type_job:
...
def is_comment_sensitive(comment_text):
    """Kiểm tra xem bình luận có chứa từ khóa nhạy cảm không."""
...
def lam_job(data, jobs, type_job_doing, current_proxy=None):
    ...
    if type_job_doing == 'review_facebook':
        ...
        if post_id_to_comment:
            # === START SỬA LỖI ===
            comment_text_to_post = jobs.get('data') # jobs ở đây là res_load
            ...
            if comment_result['status'] == 'success':
                comment_text = comment_result.get('payload', comment_text_to_post) # Lấy text thực tế
                prints(255, 255, 0, f'Bình luận thành công (dựa trên API): "{comment_text[:30]}...", chờ 10 giây...')
                time.sleep(10)
                
                # Đã xóa hàm verify_comment_presence. 
                # Chúng ta tin tưởng phản hồi 'success' từ comment_fb
                return comment_result
            else:
                return comment_result # Trả về lỗi từ comment_fb
            # === END SỬA LỖI ===
        else:
...
def main_bumx_free():
    ...
    prints(255, 255, 0, f"Cơ chế đổi cookie FB: sau {COOKIE_JOB_LIMIT} jobs hoặc {CONSECUTIVE_FAILURE_LIMIT} lỗi liên tiếp.")
...
Nhập STT các loại NV cần làm (ví dụ: 12 để làm cảm xúc và comment): ''',end='')
        ...
        try:
            ...
            if (tasks_on_current_cookie >= COOKIE_JOB_LIMIT or consecutive_failures >= CONSECUTIVE_FAILURE_LIMIT) and len(valid_cookies) > 1:
                ...
                if new_data:
                    data = new_data
                    tasks_on_current_cookie = 0
                    consecutive_failures = 0 # Reset counters on successful switch
                else:
            ...
            for type_job in list_type_job:
                if total_completed_tasks >= SO_NV: break
                
                reload(bumx_session, authorization, type_job)
                time.sleep(4)
                
                # ================== SỬA LỖI ==================
                # Gọi get_job với type_job cụ thể để lọc ở API
                jobs_for_current_type = get_job(bumx_session, authorization, type_job)
                
                # Xóa bộ lọc phía client không cần thiết
                ...
                for job in jobs_for_current_type:
                    if total_completed_tasks >= SO_NV: break
                    if (tasks_on_current_cookie >= COOKIE_JOB_LIMIT or consecutive_failures >= CONSECUTIVE_FAILURE_LIMIT):
                        break

                    if has_job_been_done(job_history, data['user_id'], job['buff_id']):
                        prints(128, 128, 128, f"Nhiệm vụ {job['buff_id']} đã được làm bởi tài khoản này, bỏ qua.")
                        continue
                    ...
                    try:
                        ...
                        # ** SENSITIVITY CHECK FOR COMMENTS **
                        if job['type'] == 'like_poster':
                            comment_content = res_load.get('data', '')
                            if is_comment_sensitive(comment_content):
                                prints(255, 165, 0, 'Bình luận chứa nội dung nhạy cảm. Báo lỗi và bỏ qua.')
    ...
    prints(5,255,0,f'Số nhiệm vụ đã bỏ qua/lỗi: {demsk}')
