...
def _parse_graphql_response(response):
    ...
    try:
        ...
        if 'errors' in response_json:
            ...
            # [YÊU CẦU 3] Phát hiện hành vi tự động
...
Nhập STT các loại NV cần làm (ví dụ: 12 để làm cảm xúc và comment): ''',end='')
    ...
    # [YÊU CẦU 2] Thêm set để theo dõi các tài khoản FB bị chặn comment
    comment_blocked_accounts = set()
    ...
    while total_completed_tasks < SO_NV:
        try:
            ...
            if (tasks_on_current_cookie >= COOKIE_JOB_LIMIT or consecutive_failures >= CONSECUTIVE_FAILURE_LIMIT) and len(valid_cookies) > 1:
                ...
                if new_data:
                    ...
                    if not valid_cookies:
                        ...
                        # [YÊU CẦU 4] Báo lỗi các NV còn lại khi hết cookie
            ...
            if not all_available_jobs:
                ...
                for type_job in list_type_job:
                    ...
                    if new_jobs:
                        ...
                        prints(255, 255, 0, f"Không có nhiệm vụ mới cho loại {type_job}.")
            ...
            # [YÊU CẦU 2] Kiểm tra xem tài khoản này có bị chặn comment không
            if job['type'] == 'like_poster' and data['user_id'] in comment_blocked_accounts:
                prints(255, 165, 0, f"Tài khoản {data['name']} đã bị chặn bình luận, báo lỗi và bỏ qua NV này.")
                report(bumx_session, authorization, job)
                demsk += 1
                continue
            
            # [YÊU CẦU 1] Báo lỗi nếu job đã làm
            ...
            try:
                ...
                else: # [YÊU CẦU 2] Xử lý lỗi spam comment
                    prints(255, 165, 0, f"Hành động thất bại ({job_result['status']}): {job_result.get('message', '')}")
                    
                    if job['type'] == 'like_poster':
                        error_msg_lower = job_result.get('message', '').lower()
                        if 'spam' in error_msg_lower or 'blocked' in error_msg_lower or 'chặn' in error_msg_lower:
                            prints(255, 0, 0, f"Phát hiện chặn bình luận cho {data['name']}. Sẽ không làm NV comment cho tài khoản này nữa.")
                            comment_blocked_accounts.add(data['user_id'])
            ...
            # [YÊU CẦU 5] Báo lỗi các NV còn lại khi người dùng dừng tool
            if all_available_jobs:
